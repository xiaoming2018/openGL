...
namespace half_edge
{
	...
	public:
		...
		void build_he_structure(const unsigned long num_vertices, vector<face*>*face,
			const unsigned long num_edges, const half_edge::edge_t* edges);
		...
		void clear(){
			...
			m_direcdted_edge2he_index.clear();
		}

		void vertex_face_neighbors(const index_t vertex_index, vector<index_t> &result) const {
			result.clear();
	...
	private:
		...
		// order_edge -> half_edge index
		typedef map<pair<index_t, index_t>, index_t> mapping;
		mapping m_direcdted_edge2he_index;
	};
}
